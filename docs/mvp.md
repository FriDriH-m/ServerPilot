...
Локальный supervisor MVP запускает только нативный `.exe` с совпадающим сохранённым
именем процесса, использует `UseShellExecute = false` и не передаёт конфигурацию в shell
или PowerShell. `.bat` безопасно отклоняется до появления отдельного ограниченного
launcher-профиля. PID перед остановкой сверяется вместе со временем запуска, путём и
именем процесса; graceful и forced ожидания ограничены timeout.
...
После bootstrap Agent выполняет heartbeat и `claim-next` в независимых последовательных
циклах: следующий запрос одного типа начинается только после завершения предыдущего и
задержки. Сетевые ошибки, `408`, `429` и `5xx` получают не более трёх retry после
первой попытки с 1/2/4-секундным exponential backoff и jitter; `401`/`403`, другие
неожиданные `4xx` и некорректный контракт API останавливают Agent без бесконечного
повтора. Agent удерживает только одну `Claimed`/`Recovery` команду и не забирает
следующую до успешного terminal report. Он сначала идемпотентно переводит команду в
