...
| Agent error discloses local details, injects control data or exhausts storage | Failure code/message are required, trimmed, control-character-free and bounded; logs omit both details and user history omits the raw message | The Agent must use stable error codes and avoid unnecessary sensitive detail |
