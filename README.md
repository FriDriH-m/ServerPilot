...
последовательно каждые 15 секунд, а состояние выбранного процесса и первая страница истории —
каждые 10 секунд. Такая частота оставляет запас в стандартном пользовательском rate limit и
не запускает новый цикл до завершения предыдущего. Принятая API команда
